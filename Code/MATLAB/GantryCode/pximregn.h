/*
 *
 *	pxi(p)8regn.h	External	27-Sep-2006
 *
 *	Copyright (C)  1990-2006  EPIX, Inc.  All rights reserved.
 *
...
#define PXIMREGWINDOW	    12	    /* window (rectangle, shifted not rotated)	*/
...
#define PXIMREGBARRELBOXF   20	    /* pincushion box, rotated, w. float parameters  */
...
struct pximregion
{
  ...
  /*pximage_s	*ip;		 * future?				    */
    ...
    size_t	length; 	/* byte length of this variable len struct  */
	...
	/*
	 * Barrel/pincushion rectangle.
	 * The width & height are nominal, true for
	 * zero distortion effect.
	 * The rectangle may be drawn offset
	 * from the len's center; i.e. the point of 0
	 * distortion need not coincide with the center
	 * of the drawn rectangle.
	 */
	struct pximregbarrelboxf
	{
	    pxyf_s  originf;	    /* added to integer origin	*/
	    float   width;
	    float   height;
	    float   angle;
	    float   aspect;
	    pxyf_s  optics;	    /* point of zero distortion */
	    int     order;
	    float   coef[2];
	} barrelboxf;
