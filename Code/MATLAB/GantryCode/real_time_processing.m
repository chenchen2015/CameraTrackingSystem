...
numberOfPos = 100;         % Number of samples to process
result(numberOfPos).gantryPos = [];
result(numberOfPos).estimatedPos12 = [];
result(numberOfPos).estimatedPos13 = [];
result(numberOfPos).estimatedPos23 = [];

figure(1);clf;             % Initialize figure 1 with 3 subplots
h1 = subplot(1,3,1);
hIm1 = imshow(rand(2048), 'Parent', h1);hold on;
hLED1 = plot(0,0,'g.','MarkerSize',24);
h2 = subplot(1,3,2);
hIm2 = imshow(rand(2048), 'Parent', h2);hold on;
hLED2 = plot(0,0,'g.','MarkerSize',24);
h3 = subplot(1,3,3);
hIm3 = imshow(rand(2048), 'Parent', h3);hold on;
hLED3 = plot(0,0,'g.','MarkerSize',24);
